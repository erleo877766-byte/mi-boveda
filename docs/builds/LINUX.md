...
docker run --privileged -v$(pwd):$(pwd) -w $(pwd) -i --rm ghcr.io/erleo877766-byte/mi-boveda:debian13-flutter3.41.9-ndkr28-go1.24.1-ruststablenightly bash -x << EOF
set -x -e
...
pushd scripts/linux
    ...
    # source ./app_env.sh mi-boveda # Uncomment this line to build mi-boveda
    ./app_config.sh
popd
flutter clean
./model_generator.sh
...
flatpak build-bundle export build/linux/current/miboveda.flatpak com.cakewallet.CakeWallet
EOF
```
...
```bash
flatpak --user install build/linux/current/miboveda.flatpak
```
